...
class LawGraphImporter:
        ...
        with self.driver.session(database='viphamhanhchinh') as ses:
            ...
            for law_key, law_data in (root.get("content") or {}).items():
                ...
                for chap_key, chap_data in chapters.items():
                    ...
                    ses.execute_write(self.create_chapter, law_key, chap_key, chap_title)
                    ses.execute_write(self.rel_law_to_chapter, law_key, chap_key)
    ...
    def _walk_level(self, ses, law, chap, node_dict, parent_type=None, parent_title=None):
        for k, v in node_dict.items():
            if k.startswith("Mục"):
                ...
                if parent_type == "Muc":
                    ses.execute_write(self.rel_muc_to_tieumuc, law, chap, parent_title, muc_title, target_rel)
                elif parent_type is None:
                    ses.execute_write(self.rel_chapter_to_node, law, chap, muc_title, target_rel)
                ...
                if parent_type == "Muc":
                    ses.execute_write(self.rel_muc_to_tieumuc, law, chap, parent_title, tm_title, "HAS_TIEUMUC")
                elif parent_type == "TieuMuc":
                    ses.execute_write(self.rel_tieumuc_to_tieumuc, law, chap, parent_title, tm_title, "HAS_SUBTM")
                ...
                ses.execute_write(self.create_dieu, law, chap, dieu_number, d_title, d_content)

                if parent_type == "Muc":
                    ses.execute_write(self.rel_muc_to_dieu, law, chap, parent_title, d_title, "HAS_DIEU")
                elif parent_type == "TieuMuc":
                    ses.execute_write(self.rel_tieumuc_to_dieu, law, chap, parent_title, d_title, "HAS_DIEU")
                else:
                    ses.execute_write(self.rel_chapter_to_node, law, chap, d_title, "HAS_DIEU")
    ...
    def create_chapter(tx, law, chap, title):
        tx.run("MERGE (c:Chapter {law:$l, key:$k}) SET c.title=$t", l=law, k=chap, t=title)

    @staticmethod
    def create_muc(tx, law, chap, title):
        tx.run("MERGE (:Muc {law:$l, parent:$c, title:$t})", l=law, c=chap, t=title)

    @staticmethod
    def create_tieumuc(tx, law, chap, title):
        tx.run("MERGE (:TieuMuc {law:$l, parent:$c, title:$t})", l=law, c=chap, t=title)
