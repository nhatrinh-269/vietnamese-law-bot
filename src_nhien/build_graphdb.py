...
class LawGraphImporter:
    ...
    def _walk_level(self, ses, law, chap, node_dict, parent_type=None, parent_title=None):
        ...
        for k, v in node_dict.items():
            ...
            # --------- ĐIỀU ---------
                ...
                dieu_number = "Điều " + k.split(" ")[1]  # Lấy số điều từ khóa (ví dụ: "18" từ "Điều 18")
                d_title = v.get("title", k)  # Lấy title của điều luật
                ...
                # Tạo node Điều với số điều, title và content
    ...
    def create_dieu(tx, law, chap, dieu_number, title, content):
        """
        Tạo node Điều với thuộc tính số điều (dieu_number), title và content.
        """
        tx.run(
            """
    ...
    def rel_dieu_to_dieu(tx, law, article_number, ref_number, rel):
        """
        Tạo mối quan hệ REFERS_TO giữa các điều dựa trên dieu_number.
        """
        tx.run(
            f"""MATCH (a:Dieu {{dieu_number:$a, law:$l}}),
